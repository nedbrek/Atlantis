...
int ARegionList::GetRegType( ARegion *pReg )
{
    ...
    if((pReg->zloc>1) && (pReg->zloc < Globals->UNDERWORLD_LEVELS+2)) {
        ...
        switch (r) {
			...
			case 1:
			case 2:
			case 3: return R_OCEAN;
			case 4: return R_CAVERN;
			case 5: return R_CERAN_CAVERN1;
			case 6: return R_CERAN_CAVERN2;
			case 7: return R_CERAN_CAVERN3;
			case 8: return R_UFOREST;
			case 9: return R_CERAN_UFOREST1;
			case 10: return R_CERAN_UFOREST2;
			case 11: return R_CERAN_UFOREST3;
			case 12:
			case 13: return R_TUNNELS;
			case 14: return R_CERAN_TUNNELS1;
			case 15: return R_CERAN_TUNNELS2;
			default: return( 0 );
        }
    }
	...
	if((pReg->zloc > Globals->UNDERWORLD_LEVELS+1) &&
	   (pReg->zloc < Globals->UNDERWORLD_LEVELS+Globals->UNDERDEEP_LEVELS+2)) {
		...
		switch(r) {
			...
			case 4:
			case 5:
			case 6: return R_GROTTO;
			case 7: return R_CERAN_GROTTO1;
			case 8:
			case 9:
			case 10: return R_DFOREST;
			case 11: return R_CERAN_DFOREST1;
			case 12:
			case 13:
			case 14: return R_CHASM;
			case 15: return R_CERAN_CHASM1;
			default: return( 0 );
		}
	}
    ...
    if( pReg->zloc == 1 ) {
        ...
        case 0: /* Arctic regions */
			if (r < 6) return R_TUNDRA;
			if (r < 12) return R_CERAN_TUNDRA1;
			...
			if (r < 46) return R_PLAIN;
			if (r < 52) return R_CERAN_PLAIN1;
			if (r < 58) return R_CERAN_PLAIN2;
			return R_CERAN_PLAIN3;

        case 1: /* Colder regions */
			if (r < 3) return R_PLAIN;
			...
			return R_CERAN_SWAMP3;
        case 2: /* Warmer regions */
			if (r < 6) return R_PLAIN;
			if (r < 10) return R_CERAN_PLAIN1;
			if (r < 14) return R_CERAN_PLAIN2;
			if (r < 18) return R_CERAN_PLAIN3;
			if (r < 20) return R_FOREST;
			if (r < 22) return R_CERAN_FOREST1;
			if (r < 24) return R_CERAN_FOREST2;
			if (r < 26) return R_CERAN_FOREST3;
			if (r < 28) return R_CERAN_MYSTFOREST;
			if (r < 30) return R_CERAN_MYSTFOREST1;
			if (r < 31) return R_CERAN_MYSTFOREST2;
			if (r < 32) return R_MOUNTAIN;
			if (r < 33) return R_CERAN_MOUNTAIN1;
			if (r < 34) return R_CERAN_MOUNTAIN2;
			if (r < 35) return R_CERAN_MOUNTAIN3;
			if (r < 37) return R_SWAMP;
			if (r < 39) return R_CERAN_SWAMP1;
			if (r < 40) return R_CERAN_SWAMP2;
			if (r < 41) return R_CERAN_SWAMP3;
			if (r < 43) return R_JUNGLE;
			if (r < 45) return R_CERAN_JUNGLE1;
			if (r < 47) return R_CERAN_JUNGLE2;
			if (r < 49) return R_CERAN_JUNGLE3;
			if (r < 53) return R_CERAN_LAKE;
			if (r < 55) return R_DESERT;
			if (r < 57) return R_CERAN_DESERT1;
			if (r < 59) return R_CERAN_DESERT2;
			if (r < 61) return R_CERAN_DESERT3;
			if (r < 63) return R_CERAN_WASTELAND;
			return R_CERAN_WASTELAND1;
        case 3: /* tropical */
			if (r < 4) return R_PLAIN;
...
int ARegionList::CheckRegionExit(ARegion *pFrom, ARegion *pTo )
{
    ...
    int chance = 0;
	if( pFrom->type == R_CAVERN   || pFrom->type == R_CERAN_CAVERN1  ||
		pFrom->type == R_CERAN_CAVERN2  || pFrom->type == R_CERAN_CAVERN3  ||
		pFrom->type == R_UFOREST  || pFrom->type == R_CERAN_UFOREST1 ||
		pFrom->type == R_CERAN_UFOREST2 || pFrom->type == R_CERAN_UFOREST3 ||
		pTo->type == R_CAVERN     || pTo->type == R_CERAN_CAVERN1    ||
		pTo->type == R_CERAN_CAVERN2    || pTo->type == R_CERAN_CAVERN3    ||
		pTo->type == R_UFOREST    || pTo->type == R_CERAN_UFOREST1   ||
		pTo->type == R_CERAN_UFOREST2   || pTo->type == R_CERAN_UFOREST3 )
    {
        chance = 25;
    }
	if( pFrom->type == R_TUNNELS  || pTo->type == R_TUNNELS  ||
		pFrom->type == R_CERAN_TUNNELS1 || pTo->type == R_CERAN_TUNNELS1 ||
		pFrom->type == R_CERAN_TUNNELS2 || pTo->type == R_CERAN_TUNNELS2)
    {
        chance = 50;
    }
	if(pFrom->type == R_GROTTO || pFrom->type == R_DFOREST ||
	   pTo->type == R_GROTTO || pTo->type == R_DFOREST ||
	   pFrom->type == R_CERAN_GROTTO1 || pFrom->type == R_CERAN_DFOREST1 ||
	   pTo->type == R_CERAN_GROTTO1 || pTo->type == R_CERAN_DFOREST1) {
		// better connected underdeeps
		chance = 40;
	}
	if(pFrom->type == R_CHASM || pTo->type == R_CHASM ||
	   pFrom->type == R_CERAN_CHASM1 || pTo->type == R_CERAN_CHASM1) {
		chance = 60;
	}
